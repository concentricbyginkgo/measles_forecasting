...
| **`binary_outbreak_threshold_per_m`** | No | Number | — (uses wrapper default: **≥ 2** on depVar scale) | When set to a numeric value *T*, [`fitOne.py`](fitOne.py) passes **`binaryLabelMetric = lambda x: x >= T`** into the model wrapper so test-set classification metrics match that incidence threshold (same rule applied to observed and predicted continuous values). Blank or missing column → framework default in [`MeaslesModelEval.defaultBinaryMetric`](MeaslesModelEval.py). CSV cannot store arbitrary callables; only this **greater-or-equal threshold** pattern is supported from metadata. |
| **`binary_outbreak_threshold`** | No | Number | Same as above | **Alias** for `binary_outbreak_threshold_per_m` if the longer name is not used (first present column wins: `binary_outbreak_threshold_per_m` is checked first). |
...
```csv
ROW_ID,country,depVar,binary_outbreak_threshold_per_m,predictor,model
